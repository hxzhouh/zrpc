...
import (
	"github.com/hxzhouh/zrpc/pkg/constant"
	"github.com/hxzhouh/zrpc/pkg/utils/xtime"
	"os"
	"path/filepath"
	"runtime"
	"strings"
...
func PrintVersion() {
	//fmt.Printf("%-8s]> %-30s => %s\n", "zrpc", xcolor.Red("name"), xcolor.Blue(appName))
	//fmt.Printf("%-8s]> %-30s => %s\n", "zrpc", xcolor.Red("appID"), xcolor.Blue(appID))
	//fmt.Printf("%-8s]> %-30s => %s\n", "zrpc", xcolor.Red("region"), xcolor.Blue(AppRegion()))
	//fmt.Printf("%-8s]> %-30s => %s\n", "zrpc", xcolor.Red("zone"), xcolor.Blue(AppZone()))
	//fmt.Printf("%-8s]> %-30s => %s\n", "zrpc", xcolor.Red("appVersion"), xcolor.Blue(buildAppVersion))
	//fmt.Printf("%-8s]> %-30s => %s\n", "zrpc", xcolor.Red("zrpcVersion"), xcolor.Blue(zrpcVersion))
	//fmt.Printf("%-8s]> %-30s => %s\n", "zrpc", xcolor.Red("buildUser"), xcolor.Blue(buildUser))
	//fmt.Printf("%-8s]> %-30s => %s\n", "zrpc", xcolor.Red("buildHost"), xcolor.Blue(buildHost))
	//fmt.Printf("%-8s]> %-30s => %s\n", "zrpc", xcolor.Red("buildTime"), xcolor.Blue(BuildTime()))
	//fmt.Printf("%-8s]> %-30s => %s\n", "zrpc", xcolor.Red("buildStatus"), xcolor.Blue(buildStatus))
}
